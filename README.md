...
- **Observability:** `@sentry/nextjs` on web/TV (`platform=web|webos` tags); `@sentry/react-native` on Expo (`platform=mobile`). Set `SENTRY_DSN` in `.env.local` and `EXPO_PUBLIC_SENTRY_DSN` in `apps/mobile/.env`. Auth tokens and emails are scrubbed from breadcrumbs. Dev probe: `GET /api/debug/sentry`.
