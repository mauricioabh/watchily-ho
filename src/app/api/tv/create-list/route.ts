...
export async function POST(request: NextRequest) {
  ...
  if (!user)
    return NextResponse.redirect(
      new URL(request.url).origin + "/login-standalone",
      302,
    );
  ...
  const redirectTo =
    ...
    new URL(request.url).origin + "/lists-standalone";

  if (!name) {
    return NextResponse.redirect(
      redirectTo + "?error=El+nombre+es+obligatorio",
      302,
    );
  }
  ...
  if (bump.error) {
    return NextResponse.redirect(
      redirectTo + "?error=" + encodeURIComponent(bump.error),
      302,
    );
  }
