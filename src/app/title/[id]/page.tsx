...
export default async function TitlePage({
  ...
  let title = await getTitleDetails(id, {
    region: userCountry,
    country: userCountry,
  });
  if (!title) notFound();
  ...
  const backdropUrl = title.backdrop?.startsWith("http")
    ? title.backdrop
    : undefined;
              ...
              {(title.imdbRating != null ||
                ...
                <div className="flex flex-wrap gap-3">
                  {title.imdbRating != null && (
                    <ScoreBadge
                      label="IMDb"
                      value={title.imdbRating.toFixed(1)}
                      color="#f5c518"
                    />
                  )}
                  {title.userRating != null && (
                  ...
                  {title.rottenTomatoesRating != null && (
                    <ScoreBadge
                      label="RT"
                      value={`${title.rottenTomatoesRating}%`}
                      color={
                        title.rottenTomatoesRating >= 60 ? "#f87171" : "#94a3b8"
                      }
                    />
                  )}
                </div>
